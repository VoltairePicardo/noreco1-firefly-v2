...
export class ItemTestingDetailComponent implements OnInit {
    ...
    loadWorkflowActions(): void {
        ...
        this.service.getWorkflowActions(this.data().transId).subscribe({ next: (a) => { this.workflowActions.set(a || []); }, error: () => { this.workflowActions.set([]); } });
    }
    ...
    toggleLogs(): void {
        ...
        if (this.showLogs && this.logs().length === 0) {
            ...
            this.service.getDocumentLogs(this.data().transId).subscribe({ next: (l) => { this.logs.set(l || []); this.logsLoading.set(false); }, error: () => { this.logsLoading.set(false); } });
        }
    }
    ...
    getTotalUnitsReceived(): number {
        return (this.data().itemTestingDetails || []).reduce((sum: number, d: any) => sum + (Number(d.unitsReceivedQuantity) || 0), 0);
    }

    getTotalAccepted(): number {
        return (this.data().itemTestingDetails || []).reduce((sum: number, d: any) => sum + (Number(d.quantityReceived) || 0), 0);
    }

    getTotalRejected(): number {
        return (this.data().itemTestingDetails || []).reduce((sum: number, d: any) => sum + (Number(d.unitsRejectedQuantity) || 0), 0);
    }
}
