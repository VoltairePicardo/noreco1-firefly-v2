...
@Component({
    selector: 'app-sl-entity-add-edit',
    imports: [...COMMON_ALL_PAGE_IMPORTS, ...COMMON_ADD_EDIT_PAGE_IMPORTS, LaddaModule],
    templateUrl: './sl-entity-add-edit.component.html'
})
export class SlEntityAddEditComponent {
    module     = 'SL Entity';
    subModule  = 'Create';
    menuLink   = 'sl-entity';
    id: any    = 0;
    ...
    loadClassifications(): void {
        this.service.getClassifications().subscribe({
            next: (data) => this.classifications.set(Array.isArray(data) ? data : []),
            error: () => this.alertService.error(this.module, 'Load Classifications', '')
        });
    }
    ...
    initForm(data?: any): void {
        this.validationForm = this.fb.group({
            ...
            slEntityClassificationId:  [data?.slEntityClassification?.id || null, Validators.required],
        });
    }
    ...
    validSubmit(): void {
        ...
        const { slEntityClassificationId, ...rest } = this.form.value;
        const frm = {
            id: this.id || 0,
            ...rest,
            slEntityClassification: { id: slEntityClassificationId }
        };
