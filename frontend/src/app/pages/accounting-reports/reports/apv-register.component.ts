...
@Component({
    ...
    templateUrl: './apv-register.component.html'
})
export class ApvRegisterComponent {
    module   = 'APV Register';
    ...
    search(): void {
        ...
        this.service.getApvRegister(this.fromDate, this.toDate, this.selectedStatusId).subscribe({
            next: (data) => { this.rows.set(data || []); this.isLoading.set(false); },
            error: () => { this.isLoading.set(false); }
        });
        this.service.getApvRegisterRecap(this.fromDate, this.toDate, this.selectedStatusId).subscribe({
    ...
    export(type: 'pdf' | 'xls'): void {
        ...
        this.downloadSvc.print(`/reports/export/apv-register/${this.fromDate}/${this.toDate}/${this.selectedStatusId}`, { type });
    }
}
