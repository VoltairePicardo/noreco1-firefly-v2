...
export class JvRegisterComponent {
    module   = 'JV Register';
    ...
    search(): void {
        ...
        this.service.getJvRegister(this.fromDate, this.toDate, this.selectedStatusId).subscribe({
            next: (data) => { this.rows.set(data || []); this.isLoading.set(false); },
            error: () => { this.isLoading.set(false); }
        });
        this.service.getJvRegisterRecap(this.fromDate, this.toDate, this.selectedStatusId).subscribe({
    ...
    export(type: 'pdf' | 'xls'): void {
        ...
        this.downloadSvc.print(`/reports/export/jv-register/${this.fromDate}/${this.toDate}/${this.selectedStatusId}`, { type });
    }
}
