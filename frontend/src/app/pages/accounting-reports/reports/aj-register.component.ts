...
@Component({
    ...
    templateUrl: './aj-register.component.html'
})
export class AjRegisterComponent {
    module   = 'AJ Register';
    ...
    search(): void {
        ...
        this.service.getAjRegister(this.fromDate, this.toDate, this.selectedStatusId).subscribe({
            next: (data) => { this.rows.set(data || []); this.isLoading.set(false); },
            error: () => { this.isLoading.set(false); }
        });
        this.service.getAjRegisterRecap(this.fromDate, this.toDate, this.selectedStatusId).subscribe({
    ...
    export(type: 'pdf' | 'xls'): void {
        ...
        this.downloadSvc.print(`/reports/export/aj-register/${this.fromDate}/${this.toDate}/${this.selectedStatusId}`, { type });
    }
}
