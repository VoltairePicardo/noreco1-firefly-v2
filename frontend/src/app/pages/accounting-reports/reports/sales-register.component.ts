...
export class SalesRegisterComponent {
    ...
    search(): void {
        ...
        this.service.getSalesRegister(this.fromDate, this.toDate, this.selectedStatusId).subscribe({
            next: (data) => { this.rows.set(data || []); this.isLoading.set(false); },
            error: () => { this.isLoading.set(false); }
        });
        this.service.getSalesRegisterRecap(this.fromDate, this.toDate, this.selectedStatusId).subscribe({
    ...
    export(type: 'pdf' | 'xls'): void {
        ...
        this.downloadSvc.print(`/reports/export/sales-register/${this.fromDate}/${this.toDate}/${this.selectedStatusId}`, { type });
    }
}
