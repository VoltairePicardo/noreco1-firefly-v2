...
export class CvRegisterComponent {
    module   = 'CV Register';
    ...
    search(): void {
        ...
        this.service.getCvRegister(this.fromDate, this.toDate, this.selectedStatusId).subscribe({
            next: (data) => { this.rows.set(data || []); this.isLoading.set(false); },
            error: () => { this.isLoading.set(false); }
        });
        this.service.getCvRegisterRecap(this.fromDate, this.toDate, this.selectedStatusId).subscribe({
    ...
    export(type: 'pdf' | 'xls'): void {
        ...
        this.downloadSvc.print(`/reports/export/cv-register/${this.fromDate}/${this.toDate}/${this.selectedStatusId}`, { type });
    }
}
