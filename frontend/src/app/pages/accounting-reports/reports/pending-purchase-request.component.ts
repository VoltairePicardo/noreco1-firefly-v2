...
@Component({
    selector: 'app-pending-purchase-request',
    ...
    templateUrl: './pending-purchase-request.component.html'
})
export class PendingPurchaseRequestComponent {
    module   = 'Pending Purchase Requests';
    ...
    statusOptions  = ['Pending', 'Approved', 'All'];
    ...
    search(): void {
        ...
        this.rows.set([]);
        this.service.getPendingPurchaseRequests(this.fromDate, this.toDate, this.selectedStatus).subscribe({
    ...
    export(type: 'pdf' | 'xls'): void {
        ...
        this.downloadSvc.print(
            `/reports/export/pending-purchase-requests`,
            { type, from: this.fromDate, to: this.toDate, status: this.selectedStatus }
        );
    }
}
