...
export class MemorandumReceiptDetailComponent {
    ...
    toggleLogs(): void {
        ...
        if (this.showLogs && this.logs.length === 0) {
            ...
            this.service.getDocumentLogs(this.transactionId!).subscribe({ next: (l) => { this.logs = l || []; this.logsLoading = false; }, error: () => { this.logsLoading = false; } });
        }
    }
