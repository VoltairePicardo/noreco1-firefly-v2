...
export class ApexchartComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        // setTimeout defers getOptions() so CSS variables from the new
        // data-bs-theme / data-skin attribute are fully resolved before
        // getComputedStyle reads them inside getColor().
        this.layoutSub = this.layout.layoutState$.pipe(skip(1)).subscribe(() => {
            setTimeout(() => {
                this.options = this.getOptions();
            });
        });
